...
class FoxManagerNode(Node):
    ...
    def killFox(self, hitFox):
        ...
        fox['death_animation'] = 'lying_down'
        self.get_logger().info(f"{hitFox} has gradually fallen and is now lying down.")
    
    def animate_death(self, fox_name, fox_data, dt):
            
        anim = fox_data['death_animation']
        if anim is None:
            return
        ...
        fx, fy, fz = anim['initial_pos']
        initial_yaw = anim['initial_yaw']
            
        # Tip over animation (rotate 90 degrees around Y axis)
        roll = t * (math.pi / 2)  # Tip to side
            
        # Apply tipped pose
        self.set_pose_with_rotation(fox_name, fx, fy, fz, roll, 0, initial_yaw)
            
        # Color fade (only update every 0.1 seconds to avoid spam)
        anim['color_progress'] += dt
        if anim['color_progress'] >= 0.1:
            anim['color_progress'] = 0.0
                
            # Interpolate from red (1.0, 0.1, 0.1) to grey (0.5, 0.5, 0.5)
            r = 1.0 - (0.5 * t)
            g = 0.1 + (0.4 * t)
            b = 0.1 + (0.4 * t)
                
            self.change_fox_color(fox_name, r, g, b)
    ...
    def change_fox_color(self, fox_name, r, g, b, alpha=1.0):
        """Change the fox's material color using Ignition visual_config service"""
        req = (
            f'entity: {{name: "{fox_name}", type: MODEL}}; '
            f'ambient: {{r: {r}, g: {g}, b: {b}, a: {alpha}}}; '
            f'diffuse: {{r: {r}, g: {g}, b: {b}, a: {alpha}}}; '
            f'specular: {{r: {r*0.3}, g: {g*0.3}, b: {b*0.3}, a: {alpha}}}; '
            f'emissive: {{r: {r*0.3}, g: {g*0.3}, b: {b*0.3}, a: {alpha}}}'
        )
            
        cmd = [
            "ign", "service", "-s", f"/world/{self.world}/visual_config",
